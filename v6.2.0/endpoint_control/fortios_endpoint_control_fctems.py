...
def filter_endpoint_control_fctems_data(json):
    option_list = ['admin_password', 'admin_username', 'https_port',
                   'name', 'serial_number', 'server',
                   'source_ip']
    dictionary = {}
...
def endpoint_control_fctems(data, fos):
    vdom = data['vdom']
    state = data['state']
...
def fortios_endpoint_control(data, fos):
    ...
    if data['endpoint_control_fctems']:
        resp = endpoint_control_fctems(data, fos)
